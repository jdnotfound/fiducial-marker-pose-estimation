...
TEST_NAME = "apriltag_dist_2.5m_angle_0deg"
...
print("====================================")
print("APRILTAG DISTANCE + ACCURACY TEST")
print("====================================")
...
while total_frames < MAX_FRAMES:
    ...
    if ids is not None and len(ids) > 0:
        ...
        if EXPECTED_ID is not None:
            matching_indices = np.where(ids.flatten() == EXPECTED_ID)[0]

            if len(matching_indices) > 0:
                chosen_index = int(matching_indices[0])
            else:
                wrong_id_frames += 1
                chosen_index = 0

        marker_id_value = int(ids[chosen_index][0])
        ...
        marker_2d_corners = corners[chosen_index][0].astype(np.float32)
    ...
    cv2.imshow("AprilTag Distance Accuracy Metrics", frame)
